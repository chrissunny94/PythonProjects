...
ballPositionMarkerColors = (200,0,0)
ballTrajectoryMarkerColors = (200,55,55)
...
def drawBallsAndTrajectory(frameCopy, matches, ballCenters, ballVelocities, ballIndices, ballCentersToPair, ballVelocitiesToPair, frameNo=0):
    ...
    centerX = ballCenters[0]
    centerY = ballCenters[1]
    velocityX = ballVelocities[0]
    velocityY = ballVelocities[1]
    cv2.circle(frameCopy, (int(centerX), int(centerY)), 6, ballPositionMarkerColors, thickness=6)
    ...
    for position in positions:
        ...
        if (position[0] < width) and (position[1] < height):
            cv2.circle(frameCopy, (int(position[0]), int(position[1])), 2, ballTrajectoryMarkerColors, thickness=2)
...
def main():
    ...
    while(cap.isOpened()):
        ...
        for color, ballIndices in zip(['orange2', 'darkYellowAttempt2(isolating)'], ([0,1], [2])):
            ...
            ballCentersToPair = ballCenters
            ballVelocitiesToPair = ballVelocities
